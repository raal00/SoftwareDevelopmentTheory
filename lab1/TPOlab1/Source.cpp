...
int main(void) 
{
	...
	cin >> choice;
	Sorter<string>* _sorter;

	switch (choice) {
	case 0:
		cout << "BUBBLE SORT" << endl;
		_sorter = new BubbleSorter<string>(SIZE);
	...
	case 1:
		...
		_sorter = new SelectionSorter<string>(SIZE);
		_sorter->Print(inputArray);
		_sorter->Sort(inputArray, cmp);
		_sorter->Print(inputArray);
		break;
	case 2:
		...
		_sorter = new InsertionSorter<string>(SIZE);
		_sorter->Print(inputArray);
		_sorter->Sort(inputArray, cmp);
		_sorter->Print(inputArray);
		break;
	case 3:
		...
		_sorter = new PyramidSorter<string>(SIZE);
		_sorter->Print(inputArray);
		_sorter->Sort(inputArray, cmp);
		_sorter->Print(inputArray);
		break;
